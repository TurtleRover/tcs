...
class Signal(Thread):
    ...
    def getIP(self, ifname="wlan0"):
        ...
        ip = None
        try:
            ip = fcntl.ioctl(s.fileno(), 0x8915, struct.pack('256s', bytes(ifname[:15], 'utf-8')) )[20:24]
            return socket.inet_ntoa(ip)
        except OSError as e:
            logger.error('%s: %s at %s', e, ip, ifname)
            return None
    ...
    def spawn_ping(self):
        try:
            ip = self.getIP()
            self.process = subprocess.Popen(['ping', '-i', '0.05', ip, '-f'], stdout=subprocess.PIPE)
        except BaseException:
            logger.error('ping -i 0.05 %s -f', ip)
    ...
    def run(self):
        ...
        try:
            ...
            logger.error('Signal thread not started')
