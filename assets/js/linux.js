...
var linux = (function () {
    ...
    function startPythonServer() {
        ...
        if (DEBUG)
            console.log("Image resolution: " + res);
            ...
            function (data) {
                ...
                if (numberOfClients >= 0 || document.domain.includes("localhost")) {
                    ...
                    if ($.isNumeric(serverProcessPID) || $.isNumeric(mjpegStreamPID)) {
                        ...
                        if (DEBUG)
                            console.log("serverProcessPID: " + serverProcessPID);
                        if (DEBUG)
                            console.log("mjpegStreamPID: " + mjpegStreamPID);
                    ...
                    // Otherwise display a notice and work with static image
                        ...
                        if (DEBUG)
                            console.log("couldn't initialize python server");
                    ...
                    amplify.publish("all->ui", "notifications.client-already-connected");
            });
    };
    ...
    function retIsCameraAvailable() {
        return isCameraAvailable;
    };
    
    function retServerProcessPID() {
    ...
    function controllerMessageCallback(message) {
        if (DEBUG)
            console.log("controllerMessageCallback: " + message);
    ...
    function webrtcMessageCallback(message) {
        if (DEBUG)
            console.log("webrtcMessageCallback: " + message);

        switch (message) {
            case "camera stream is ready":
                //  wait until communication channel is ready
                while (communicationEstablished === false && !document.domain.includes("localhost"))
